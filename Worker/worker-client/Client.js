...
function doWork(msg){
    ...
    if(receivedmsg[0] == 'products'){

      //  productmodel.find({})
      // .exec()
      // .then((products) => {
      //   console.log(products);
      //   sendDataToQueue(JSON.stringify(products)).catch(console.error);  
      // })
      // .catch((err) =>{
      //   sendDataToQueue('error ocurred trying to fetch the products!').catch(console.error);
      // });

      sendDataToQueue(JSON.stringify(jsonProducts)).catch(console.error);
      
    }

    else if(receivedmsg[0] == 'add'){

       product.name = receivedmsg[1];
       product.amount = receivedmsg[2];
       product.price = jsonProducts[receivedmsg[1]].price;


       var productInfo = [];
          productInfo[0] = 'amount:';
          productInfo[1] = product.amount
          productInfo[2] = 'price:';
          productInfo[3] = jsonProducts[receivedmsg[1]].price;
          jsonCart.products[receivedmsg[1]] = productInfo;
          jsonCart.info.total += (productInfo[1] * productInfo[3]); 



      sendDataToQueue("The product added to the cart was:\n"+JSON.stringify(product)).catch(console.error);
    }

    else if(receivedmsg[0] == 'cart'){

        sendDataToQueue(JSON.stringify(jsonCart)).catch(console.error);

    }

    else if(receivedmsg[0] == 'delete'){

        var name = receivedmsg[1];

        var price = jsonCart.products[name][3] * jsonCart.products[name][1];
         
        jsonCart.info.total -= price; 
        delete jsonCart.products[name];


        sendDataToQueue("The product deleted from the cart was: " + name).catch(console.error);

    }
    else if(receivedmsg[0] == 'error'){
        sendDataToQueue("error - request not recognized").catch(console.error);      
    }
}
